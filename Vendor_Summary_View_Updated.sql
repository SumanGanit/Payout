select cbsv.pc_code,
cbsv.payable_amount as commercial_payables,
coalesce(hbsv.payable_amount,0) as hvdi_payables,
coalesce(slt.sla_paid,0) as incentive_penalty_payables,
commercial_payables+hvdi_payables+incentive_penalty_payables as total_payables
from
((select pc_code,
...
payout_type
from
(select distinct vendor_code as pc_code,
sum(payable_amount) as payable_amount,
payout_type
from dev_payout_report.vehicle_summary_view_test
where payout_type = 'Commercials'
group by pc_code,model,payout_type)
union
...
where payout_type = 'Commercials'
group by pc_code,model,payout_type))
group by pc_code)cbsv
left join (select pc_code,
sum(payable_amount) as payable_amount
from 
(select pc_code,
payable_amount,
...
where payout_type = 'HVDI'
group by pc_code,model,payout_type))
group by pc_code)hbsv on hbsv.pc_code = cbsv.pc_code
left join(select pc_code,
...
paymonth))
group by vendor_code,
paymonth)com on com.vendor_code = pc_code)slt on slt.pc_code = cbsv.pc_code)