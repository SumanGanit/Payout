select
    ...
    vendor_name,
    sum(no_of_consignments_delivered) as no_of_consignments_delivered,
    sum(no_of_consignments_eligible) as no_of_consignments_eligible,
    sum(no_of_consignments_delivered) - sum(no_of_consignments_eligible) as no_of_consignments_failed_eligibility,
    count(distinct delivery_date) as days_delivered,
    pay_out_month,
    pay_out_year,
    sum(commercials) as commercial_payable,
    sum(incentive) as incentives,
    sum(penalty - coalesce(cod_penalty, 0)) as penalty,
    sum(net_payables) as total_payables
from
    (
        (
            select
                distinct nd.vendor_code,
                nd.vendor_name,
                count(distinct nd.consignment_number) as no_of_consignments_delivered,
                sum(
                    case
                        when nd.eligible = 'Yes' then 1
                        else 0
                    end
                ) as no_of_consignments_eligible,
                nd.category_model,
                nd.delivery_date,
                nd.pay_out_month,
                nd.pay_out_year,
                nd.product,
                sum(nd.commercial) as commercials,
                (
                    greatest(0, nd.fad_percent) + greatest(0, nd.td_percent) + greatest(0, nd.scd_percent)
                ) as incentive_percent,
                sum(
                    nd.fad_incentive + nd.td_incentive + nd.scd_incentive
                ) as incentive,
                (
                    least(0, nd.fad_percent) + least(0, nd.td_percent) + least(0, nd.scd_percent)
                ) as penalty_percent,
                sum(nd.fad_penalty + nd.td_penalty + nd.scd_penalty) as penalty,
                sum(coalesce(nd.codpenalty, 0)) as cod_penalty,
                sum(nd.net_payable) as net_payables
            from
                (
                    select
                        ...
                        ndv.dox_type,
                        ndv.category_model,
                        cast(ndv.delivery_date as date) as delivery_date,
